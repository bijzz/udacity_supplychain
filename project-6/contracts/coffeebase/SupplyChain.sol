...
contract SupplyChain is ConsumerRole, DistributorRole, FarmerRole, RetailerRole {
  ...
  modifier received(uint _upc) {
    require(items[_upc].itemState == State.Received, "Package is not in status received.");
    _;
  }
  ...
  onlyFarmer
  {
    ...
    item.productID = sku + _upc; // unique
  ...
  function buyItem(uint _upc) public payable 
    ...
    // only for solidity >= 5
    //address payable farmerAddressPayable = _make_payable(farmerAddress); // We need to make this conversion to be able to use transfer() function to transfer ethers
